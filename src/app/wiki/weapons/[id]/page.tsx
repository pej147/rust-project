      ...
      <main className="pt-24 pb-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2 text-sm text-rust-text-muted mb-6">
            ...
            <span>/</span>
            <Link href="/wiki/weapons" className="hover:text-rust-text">Weapons</Link>
            <span>/</span>
            <span className="text-rust-text">{weapon.name}</span>
          </div>

          {/* Weapon Header */}
          <div className="flex items-start gap-4 mb-8">
            <div className="w-16 h-16 bg-rust-surface rounded-lg border border-rust-border flex items-center justify-center text-3xl">
              {categoryInfo.emoji}
            </div>
            <div>
              ...
              {!isAmmo && (
                <div className="flex items-center gap-4 mt-2">
                  ...
                  {weapon.range && (
                    <span className="text-rust-text-secondary">{weapon.range}</span>
                  )}
                  {weapon.rateOfFire && (
                    <span className="text-rust-text-secondary">{weapon.rateOfFire} RPM</span>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="bg-rust-surface rounded-lg p-6 border border-rust-border mb-6">
            <p className="text-rust-text leading-relaxed">{weapon.description}</p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="bg-rust-surface rounded-lg p-6 border border-rust-border">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-rust-text-secondary">Damage</span>
                    <span className="text-red-400 font-bold">{weapon.damage}</span>
                  </div>
                  {weapon.rateOfFire && (
                    <div className="flex justify-between items-center">
                      <span className="text-rust-text-secondary">Rate of Fire</span>
                      <span className="text-white">{weapon.rateOfFire} RPM</span>
                    </div>
                  )}
                  {weapon.range && (
                    <div className="flex justify-between items-center">
                      <span className="text-rust-text-secondary">Range</span>
                      <span className="text-white">{weapon.range}</span>
                    </div>
                  )}
                  {weapon.ammo && (
                    <div className="flex justify-between items-center">
                      <span className="text-rust-text-secondary">Ammo Type</span>
                      <span className="text-white">{weapon.ammo}</span>
                    </div>
                  )}
                  {weapon.magazineSize && (
                    <div className="flex justify-between items-center">
                      <span className="text-rust-text-secondary">Magazine</span>
            ...
            {weapon.crafting && (
              <div className="bg-rust-surface rounded-lg p-6 border border-rust-border">
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-xs text-rust-text-muted uppercase mb-2">Ingredients</div>
                    {weapon.crafting.ingredients.map((ing, i) => (
                      <div key={i} className="text-rust-text flex justify-between">
                        <span>{ing.item}</span>
                        <span className="text-rust-text-muted">x{ing.amount}</span>
                      </div>
                    ))}
                  </div>
                  <div className="flex justify-between items-center pt-3 border-t border-rust-border">
                    <span className="text-rust-text-secondary">Workbench</span>
                    <span className="text-white">
                      {weapon.crafting.workbench === 0 ? "None" : `Level ${weapon.crafting.workbench}`}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-rust-text-secondary">Blueprint</span>
                    <span className={weapon.crafting.blueprint ? "text-yellow-400" : "text-green-400"}>
                      {weapon.crafting.blueprint ? "Required" : "Default"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-rust-text-secondary">Craft Time</span>
              ...
              <div className="bg-rust-surface rounded-lg p-6 border border-rust-border">
                ...
                <div className="flex flex-wrap gap-2">
                  {weapon.attachments.map((att, i) => (
                    <span key={i} className="text-sm bg-rust-surface-elevated text-rust-text px-3 py-1 rounded-full">
                      {att}
                    </span>
                  ))}
                </div>
